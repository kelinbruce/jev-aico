#!/usr/bin/env python3
"""Minimal Jev API request demo using only the Python standard library."""
...
def ask_jev(state: str) -> dict:
    payload = {
        ...
        "questions": {
            "is_urgent": {
                "type": "noul",
                "instructions": "Does this message describe an urgent issue?",
            }
        },
    }
    request = Request(
...
def main() -> int:
    state = " ".join(sys.argv[1:]).strip() or (
        "The production service is unavailable and all customers are affected."
    )
    try:
        result = ask_jev(state)
